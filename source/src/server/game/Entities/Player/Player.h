...
#include "../scripts/Custom/Template/Template.h"
...
struct Keystone
{
    uint32 mapId = 0;
    uint8 level = 1;
    std::vector<uint8> affixes = { 0, 0, 0, 0 };
};
...
struct PvPInfo
{
    PvPInfo() : inHostileArea(false), inNoPvPArea(false), inFFAPvPArea(false), endTimer(0), pvpFlagStatus(0) {}
...
struct DuelInfo
{
    DuelInfo() : initiator(NULL), opponent(NULL), startTimer(0), startTime(0), outOfBound(0), isMounted(false), accepted(false) {}
...
struct WarGameRequestInfo
{
    WarGameRequestInfo() : opponent(NULL), startTime(0) {}
...
enum PlayerFlags
{
    /*
        ...
        PLAYER_FLAGS is sandwiched between duel arbiter and guild rank. by adding negatives into this enum, we unlock some weird client side behavior possibilities.
    */
    ...
    PLAYER_FLAGS_PVP_TOGGLE_STATUS      = 0x00000200,
    ...
    PLAYER_FLAG_COMMENTATOR             = 0x00080000,       // COMMENTATOR LABEL
    ...
    PLAYER_FLAGS_UNK31                  = 0x80000000,        //White screen, "pvp combat toggled on", pretty much everything happens.

    //combinations
    PLAYER_FLAGS_ISOLATED_BY_GM         = (PLAYER_FLAGS_UNK31 | PLAYER_FLAGS_GHOST | PLAYER_FLAGS_IS_OUT_OF_BOUNDS),
};
...
enum PlayerLoginQueryIndex
{
    ...
    PLAYER_LOGIN_QUERY_LOAD_KEYSTONES               = 47,
    PLAYER_LOGIN_QUERY_LOAD_ARENA_REWARD            = 48,
    PLAYER_LOGIN_QUERY_LOAD_CLIENT_CONFIGS          = 49,
...
enum PlayerPvPEvents
{
    PVP_EVENT_DOFLAG = 0,
    PVP_EVENT_REQUEST_REMOVEFLAG,
    PVP_EVENT_RESET_REQUEST_TIME,
    PVP_EVENT_EXECUTE_REMOVEFLAG,
    PVP_EVENT_ENTER_SANCTUARY,
};

enum PlayerPvPStates
{
    PVP_STATE_IN_PVP_NORMAL,
    PVP_STATE_IN_PVP_FFA,
    PVP_STATE_IN_PVP_BGS,
    PVP_STATE_REQUESTING_LEAVE,
    PVP_STATE_OUT_OF_PVP,
};
...
class Player : public Unit, public GridObject<Player>
{
    ...
    public:
        ...
        void GetCustomizationSettings(uint8& hairStyle, uint8& skinType, uint8& facialHair, uint8& faceType, uint8& hairColor);
        void SetORace() { m_RealRace = GetByteValue(UNIT_FIELD_BYTES_0, 0); }; // SHOULD ONLY BE CALLED ON LOGIN
        uint16 get_Ideal_Crossfaction_Model();
        void SetFakeRace(); // SHOULD ONLY BE CALLED ON LOGIN
        uint8 getFRace() const { return m_FakeRace; }
        uint32 GetFakeMorph() const { return m_FakeMorph; }
        ...
        static bool BuildEnumData(PreparedQueryResult result, ByteBuffer* dataBuffer, ByteBuffer* bitBuffer, WorldSession* session);
        ...
        void CarbonCopy(uint8 source, std::string const& channel, std::string const& what);
        bool IsFalling() { return ((GetPositionZ() < m_lastFallZ) && (m_movementInfo.HasMovementFlag(MOVEMENTFLAG_FALLING | MOVEMENTFLAG_FALLING_FAR) || movespline->isFalling())); }
        ...
        uint32 GetRole(bool set_to_group);
        ...
        void SendSniffReport(std::string const& text);
        ...
        Unit* GetPetSummoned(uint32 petEntry);
        ...
        bool HasReputationForItem(ItemTemplate const* pProto) const;
		...
		void UnequipItem(Item* pItem, bool update = true);
        ...
        */
        ...
        void DestroyItemByGUID(uint64 guid, bool update);
        ...
        void LoadCorpse();
        void LoadPets(int8 db_saved_slot = 0);
        ...
        void KilledMonsterCredit(uint32 entry, uint64 guid = 0, uint16 count = 1);
        ...
        bool ShouldSpellOverride(SpellInfo const* spell_1, SpellInfo const* spell_2);
		...
		bool LearnTalentByForce(uint32 talentId, uint32 talentRank);
        ...
        void DuelCooldownReset(Player* challenger);
        ...
        PvPInfo pvpInfo;
        void ExecutePvPStatusEvent(uint8 eventType);
        void UpdatePvPState(bool onlyFFA = false);
        void UpdatePvP(bool state, bool force_skip_five_minutes=false);
        void UpdateZone(uint32 newZone, uint32 newArea);
        bool IsPVPForceFlaggedZone(uint32 zone);
        ...
        WarGameRequestInfo* wargameRequest;
        ...
        std::string GetGuildRankName() const;
        ...
        void SavePetFromCreatureTemplate(Creature* creature, PetSlot stable_slot, uint32 spell_id);
        ...
        void SetSkipOnePacketForASH(bool blinked);
        ...
        bool UnderACKmountAdvanced() const { return m_ACKmounted_Advanced; }
        bool UnderACKmountTransport() const { return m_ACKmounted_Transport; }
        bool IsAllowedToFall() const { return m_AllowedToFall; }
        bool UnderACKRootUpd() const { return m_rootUpd; }
        void SetIsAllowedToFall(bool trueOrFalse);
        void SetUnderACKmount();
        void SetUnderACKmountAdvanced();
        void SetUnderACKmountTransport();
        void SetRootACKUpd(uint32 delay);
        uint32 GetLastJump() const { return lastJump; };
        void SetLastJump();
        ...
        void SetSuccessfullyLanded() { m_antiNoFallDmgLastChance = false; m_AllowedToFall = false; }
        ...
        void UpdatePrimaryProfessionSlots(uint32 profession_id, bool remove);
        void SetSkillFields(uint16 field, uint8 offset, uint16 skill_id, uint16 skill_step, uint16 skill_rank, uint16 skill_max_rank, uint16 skill_modifier = 0, uint16 skill_talent = 0);
        void getSkillFieldAndOffset(uint32 skill, uint16& field, uint8& offset);

        void SetSkill(uint16 id, uint16 step, uint16 currVal, uint16 maxVal);
        void SetSkill_Add(uint16 id, uint16 step, uint16 currVal, uint16 maxVal);
        void SetSkill_Update(uint16 id, uint16 step, uint16 currVal, uint16 maxVal);
        void SetSkill_Remove(uint16 id, uint16 step, uint16 currVal, uint16 maxVal);

        uint32 FindFirstOpenSkillSlot(uint32 skillid);
        uint16 CalculateSkillStep(uint32 skill);
        uint32 CalculateSkillBonus_Perm(uint32 id);
        uint32 CalculateSkillBonus_Temp(uint32 id);
        ...
        bool HasSkill(uint32 skill, bool aboveZero = true) const;
        ...
        uint32 getRacialFaction(uint8 race) const;
        ...
        bool BlockChatDueToBadgeRequirement(ChatMsg chat_type, Player* target) const;
        bool CanSpeak(bool also_check_for_shadowmute = false) const;
        ...
        */
        ...
        InstanceTimeMap* GetInstanceTimes() { return &_instanceResetTimes; };
        ...
        void SetCurrentPetSlot(int8 new_slot) { m_currentPetSlot = new_slot; };
        int8 GetCurrentPetSlot() { return m_currentPetSlot; };
        ...
        void ResetAchievementCriteriaByEntry(uint32 entry, bool evenIfCriteriaComplete = false);
        ...
        */
        ...
        VoidStorageItem* GetVoidStorageItemID(uint64 id) const;
        ...
        struct FakeMorphData
        {
            FakeMorphData();
            FakeMorphData(uint32 displayid, uint8 skin, uint8 face, uint8 hairstyle, uint8 haircolor, uint8 facialhair, uint8 category, float scale);
        ...
        FakeMorphData fakeSelfMorph;
        bool player_is_previewing_morph_model{ false };
        FakeMorphData preview_morph;
        void SetfakeSelfMorph(FakeMorphData const& morphData, bool preview_only = false);
    ...
    public:
        ...
        Keystone const& GetKeystone() { return m_keystone; }
        void SetKeystone(uint32 mapId, uint8 level);
        void RemoveKeystone();

        ArenaReward const& GetPendingArenaReward() { return m_arenareward; }
        void RemovePendingArenaReward();
    ...
    protected:
        ...
        */
        ...
        void _LoadKeystones(PreparedQueryResult result);
        void _LoadArenaReward(PreparedQueryResult result);
        void _LoadTemplates();
        ...
        void _SaveKeystones(SQLTransaction& trans);
        ...
        Difficulty m_dungeonDifficulty{ Difficulty(0) };
        Difficulty m_raidDifficulty{ Difficulty(0) };
        Difficulty m_raidMapDifficulty{ Difficulty(0) };

        uint32 m_atLoginFlags{ 0 };

        Item* m_items[PLAYER_SLOTS_COUNT]{ nullptr };
        uint32 m_currentBuybackSlot{0};
        int8 m_currentPetSlot{0};

        PlayerCurrenciesMap _currencyStorage{ PlayerCurrenciesMap(0) };
        ...
        */
        ...
        uint32 m_GuildIdInvited{ 0 };
        uint32 m_ArenaTeamIdInvited{ 0 };

        PlayerMails m_mail;
        std::map<uint32, Mail*> mails;
        PlayerSpellMap m_spells;
        uint32 m_lastPotionId{ 0 };                              // last used health/mana potion in combat, that block next potion use
...
//        Spell* m_spellModTakingSpell;  // Spell for which charges are dropped in spell::finish
    ...
    private:
        ...
        bool m_ACKmounted;
        bool m_ACKmounted_Advanced;
        bool m_ACKmounted_Transport;
        bool m_initial_tele{ false };
        bool m_AllowedToFall;
        bool m_rootUpd;
        bool m_antiNoFallDmg;
        bool m_antiNoFallDmgLastChance;
        uint32 m_mountTimer;
        uint32 m_advancedTimer;
        uint32 m_transportTimer;
        ...
        InstanceTimeMap _instanceResetTimes{ InstanceTimeMap(NULL) };
        uint32 _pendingBindId{0};
        uint32 _pendingBindTimer{0};

        bool spectatorFlag{false};
        bool spectateCanceled{false};
        Unit* spectateFrom{nullptr};

        uint32 m_CustomXpRate{7};

        uint32 _activeCheats{0};
        uint32 _maxPersonalArenaRate{0};

        uint32 _groupOutOfRangeUpdateTime{0};

        uint8 _arenaTeamInfoSlot[3]{0,0,0};

        PhaseMgr phaseMgr{PhaseMgr(NULL)};

        bool m_IsWatchingCinematic{false};

        bool m_IsWatchingMovie{false};

        bool _hasValidLFGLeavePoint{false};

        bool m_IsMovieMaker{false};

        std::map<uint32, bool > _questReported;

        uint8 _tmpLfgRolesCheck{ 0 };
        int32 m_comboPointTimer{0};
